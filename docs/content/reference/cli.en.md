...
| `start`            | starts the full backend + MCP (`/mcp`) + the SPA (fullstack)           |
| `start --headless` | starts without opening a window (backend + MCP + tray)                 |
...
vectora storage migrate memory-to-langgraph  # old memories → BaseStore
```
