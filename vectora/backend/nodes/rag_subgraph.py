...
async def _call_vector_search(
    ...
    from backend.tools.rag import vector_search  # importação local para evitar ciclo
...
async def _call_web_search(query: str) -> list[dict[str, Any]]:
    ...
    from backend.tools.web import web_search  # importação local para evitar ciclo
...
async def rag_websearch(state: State) -> dict:
    ...
    from backend.nodes.web_curation import curate_and_enqueue
