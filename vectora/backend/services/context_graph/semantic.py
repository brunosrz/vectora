...
def _parse_llm_json(raw: str) -> dict[str, Any]:
    if len(raw) > _LLM_JSON_MAX_BYTES:
        logger.warning("semantic: resposta LLM excede %d bytes — chunk descartado", _LLM_JSON_MAX_BYTES)
...
def _response_is_hollow(raw_content: str | None, parsed: dict[str, Any]) -> bool:
    if raw_content is None or not raw_content.strip():
        return True
    return not parsed.get("nodes") and not parsed.get("edges") and not parsed.get("hyperedges")
...
_CONTEXT_EXCEEDED_MARKERS = (
    ...
    "exceeds the available", "maximum context", "too many tokens",
    ...
    *,
    ...
    try:
        ...
        if not _looks_like_context_exceeded(exc):
            ...
            return {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0, "finish_reason": "error"}
        if _depth >= max_depth:
            logger.warning("semantic: chunk excede contexto em profundidade %d/%d — descartado", _depth, max_depth)
            return {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0, "finish_reason": "stop"}
        mid = len(chunk) // 2
        if mid == 0:
            if len(chunk) == 1 and isinstance(chunk[0], FileSlice):
                ...
                if halves:
                    left, right = await asyncio.gather(
                        _extract_chunk_with_retry([halves[0]], root, model_id=model_id, deep=deep, max_depth=max_depth, _depth=_depth + 1),
                        _extract_chunk_with_retry([halves[1]], root, model_id=model_id, deep=deep, max_depth=max_depth, _depth=_depth + 1),
                    )
                    return _merge_results(left, right)
            return {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0, "finish_reason": "stop"}
        left, right = await asyncio.gather(
            _extract_chunk_with_retry(chunk[:mid], root, model_id=model_id, deep=deep, max_depth=max_depth, _depth=_depth + 1),
            _extract_chunk_with_retry(chunk[mid:], root, model_id=model_id, deep=deep, max_depth=max_depth, _depth=_depth + 1),
        )
        return _merge_results(left, right)
    ...
    # Truncamento — bisect
    if _depth >= max_depth:
        logger.warning("semantic: truncamento em profundidade %d/%d — usando resultado parcial", _depth, max_depth)
        return result
    if len(chunk) <= 1:
