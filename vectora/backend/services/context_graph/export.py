...
_BACKUP_ARTIFACTS = [
    ...
    "GRAPH_REPORT.md",
    ".vectora_labels.json",
    ".vectora_analysis.json",
    "manifest.json",
    ".vectora_semantic_marker",
    "cost.json",
]
...
def backup_if_protected(out_dir: Path) -> Path | None:
    ...
    - .vectora_semantic_marker is present (graph cost real LLM tokens), or
    - .vectora_labels.json contains at least one non-default community label
    ...
    Set GRAPH_NO_BACKUP=1 to disable.
    """
    if os.environ.get("GRAPH_NO_BACKUP"):
        return None
    out = Path(out_dir)
    if not (out / "graph.json").exists():
        return None

    is_semantic = (out / ".vectora_semantic_marker").exists()
    is_curated = False
    labels_file = out / ".vectora_labels.json"
...
def _yaml_str(s: str) -> str:
    ...
    See `ingest._yaml_str` for the full rationale; duplicated here to
...
def _viz_node_limit() -> int:
    """Return the effective viz node limit, honoring GRAPH_VIZ_NODE_LIMIT env var.
        ...
        try:
            ...
            if new_n < existing_n:
                logger.warning(
                    ...
                    "Run a full rebuild (/context graph .) to be safe, or pass force=True "
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /context graph", ""]
...
def to_html(
    ...
    if G.number_of_nodes() > limit:
        ...
        raise ValueError(
            ...
            f"(limit: {limit}). Use --no-viz, raise GRAPH_VIZ_NODE_LIMIT, "
...
def _cap_filename(s: str, limit: int = 200) -> str:
    ...
    filesystem limit even after the ``.md`` extension and dedup suffix are added. The cap is on BYTES, not chars, because a label of multibyte
...
def to_obsidian(
    ...
    # Map file_type → context graph tag
    _FTYPE_TAG = {
        "code": "context graph/code",
        "document": "context graph/document",
        "paper": "context graph/paper",
        "image": "context graph/image",
    }
...
def to_canvas(
    ...
    # into one synthetic community so the canvas always reflects the graph.
...
def push_to_falkordb(
    ...
    identical to push_to_neo4j. Differences from the Neo4j path:
      ...
      - a named graph is selected via db.select_graph(graph_name) (default
        "context graph"); FalkorDB keys each graph by name in the same instance.
