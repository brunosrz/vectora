...
from backend.services.queue import EmbeddingQueueRecord, get_embedding_queue
...
class _CohereRateLimitInterceptor(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if "429" in msg or "TooManyRequests" in msg or "rate" in msg.lower():
            ...
            file_logger = logging.getLogger("backend.services.background")
            file_logger.debug(
...
class BackgroundEmbeddingWorker:
    ...
    async def _run_curator(self, workspace_id: str) -> None:
        ...
        async with lock:
            try:
                from backend.agents.rag import curate_workspace_knowledge
