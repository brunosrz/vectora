...
def _surprise_score(
    ...
    if (
        _top_level_dir(u_source) != _top_level_dir(v_source)
...
def _cross_file_surprises(
    ...
    for u, v, data in G.edges(data=True):
        ...
        score, reasons = _surprise_score(
            G, u, v, data, node_community, u_source, v_source, degrees
        )
        ...
        candidates.append(
            {
                ...
                "why": "; ".join(reasons)
                if reasons
                else "cross-file semantic connection",
            }
        )
