...
from backend.agent.docs_graph import docs_agent
from backend.middleware.retry_middleware import ModelRetryMiddleware
...
def test_retry_middleware_is_in_agent_middleware_list():
    ...
    assert any(isinstance(m, ModelRetryMiddleware) for m in middleware_list), (
        f"ModelRetryMiddleware not found in docs_agent middleware. Found: {types}"
    )
