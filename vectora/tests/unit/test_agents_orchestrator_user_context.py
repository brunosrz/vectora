...
from backend.types import CoderResult, OrchestratorDecision, SearchResult
...
class TestOrchestratorMainPath:
    ...
    async def test_injects_user_context_when_provided(self):
        ...
        with patch(
            "backend.agents.orchestrator._get_orchestrator_llm",
...
class TestSynthesizeAfterCoder:
    ...
    async def test_injects_user_context(self):
        ...
        with patch(
            "backend.agents.orchestrator._get_synthesis_llm", return_value=fake_llm
        ):
