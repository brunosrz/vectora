...
class TestRagRetrieve:
    ...
    async def test_returns_docs(self):
        ...
        with patch(
            "backend.nodes.rag_subgraph._call_vector_search_all",
    ...
    async def test_no_results(self):
        ...
        with (
            ...
            patch("backend.nodes.rag_subgraph.settings") as ms,
        ):
            m.return_value = []
...
class TestCallVectorSearchAll:
    ...
    async def test_merges_all_collections_and_tags_web(self):
        ...
        with (
            patch(
                "backend.nodes.rag_subgraph._list_collections",
                new_callable=AsyncMock,
            ) as mlist,
            patch("backend.nodes.rag_subgraph._call_vector_search", side_effect=fake),
        ):
            mlist.return_value = [
...
class TestRagWebsearch:
    ...
    async def test_adds_curated_web_docs(self):
        ...
        with patch(
            "backend.nodes.rag_subgraph._call_web_search", new_callable=AsyncMock
        ) as m:
            with patch(
                "backend.nodes.web_curation.curate_and_enqueue",
                new_callable=AsyncMock,
            ) as mc:
                m.return_value = web
...
class TestResultScore:
    ...
    def test_relevance_score_used_directly(self):
        from backend.nodes.rag_subgraph import _result_score
...
class TestCallVectorSearch:
    ...
    async def test_returns_empty_on_error_status(self):
        ...
        with patch("backend.nodes.rag_subgraph.vector_search", create=True):
            with patch(
                "backend.tools.rag.vector_search",
    ...
    async def test_returns_docs_on_success(self):
        ...
        with patch("backend.tools.rag.vector_search") as mock_vs:
...
class TestCallWebSearch:
    ...
    async def test_returns_list_on_success(self):
        ...
        with patch("backend.tools.web.web_search") as mock_ws:
...
class TestRagRerank:
    ...
    async def test_no_api_key_returns_empty(self):
        ...
        with patch("backend.nodes.rag_subgraph.settings") as ms:
