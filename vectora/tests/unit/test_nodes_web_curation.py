...
from backend.nodes.web_curation import (
    curate_and_enqueue,
    curate_web_results,
)
from backend.types import WebResultVerdict
...
class TestCurateWebResults:
    ...
    async def test_reranker_filters_below_threshold(self):
        ...
        with patch(
            "backend.nodes.web_curation._rerank_results", new_callable=AsyncMock
        ) as mr:
            with patch(
                "backend.nodes.web_curation._judge", new_callable=AsyncMock
            ) as mj:
    ...
    async def test_kill_switch_approves_all(self):
        results = _results()
        with patch("backend.nodes.web_curation.settings") as ms:
...
class TestCurateAndEnqueue:
    ...
    async def test_enqueues_only_approved(self):
        ...
        with patch(
            "backend.nodes.web_curation.curate_web_results", new_callable=AsyncMock
        ) as mc:
            mc.return_value = (approved, rejected)
            with patch("backend.tools.rag.embedding") as me:
