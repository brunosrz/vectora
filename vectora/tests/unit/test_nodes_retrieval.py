...
from backend.nodes.retrieval import _rerank, retrieval_node
...
class TestRetrievalNode:
    ...
    async def test_returns_rag_docs_on_success(self):
        ...
        with patch(
            ...
            with patch(
                "backend.nodes.retrieval._rerank", new_callable=AsyncMock
            ) as mock_rerank:
...
class TestRerank:
    ...
    async def test_rerank_success_returns_reranked(self):
        ...
        from backend.settings import settings as real_settings
