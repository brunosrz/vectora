...
from backend.graph import ACCEPT_EDITS_AUTO, REQUIRE_APPROVAL, hitl_check
...
async def test_hitl_check_terminal_approve():
    ...
    with patch("backend.graph.interrupt", return_value={"action": "approve"}):
        result = await hitl_check(state)
...
async def test_hitl_check_terminal_approve_bare_string():
    ...
    with patch("backend.graph.interrupt", return_value="approve"):
        result = await hitl_check(state)
...
async def test_hitl_check_terminal_approve_empty_string():
    ...
    with patch("backend.graph.interrupt", return_value=""):
        result = await hitl_check(state)
...
async def test_hitl_check_terminal_reject():
    ...
    with patch("backend.graph.interrupt", return_value={"action": "reject"}):
        result = await hitl_check(state)
...
async def test_hitl_check_interrupt_receives_payload():
    ...
    with patch("backend.graph.interrupt", side_effect=_fake_interrupt):
...
async def test_bypass_mode_skips_interrupt():
    ...
    with patch("backend.graph.interrupt", side_effect=_boom):
...
async def test_accept_edits_mixed_only_confirms_terminal():
    ...
    with patch("backend.graph.interrupt", side_effect=_fake):
        result = await hitl_check(state, _cfg("accept_edits"))
...
async def test_unknown_mode_falls_back_to_ask():
    ...
    state = _make_state([_tc("terminal", {"command": "ls"})])
    with patch("backend.graph.interrupt", return_value={"action": "approve"}) as it:
