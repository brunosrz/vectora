...
def test_get_prefs_vazio_por_padrao():
    out = asyncio.run(get_prefs(_FakeRequest("u1")))
    assert out == {}


def test_update_e_get_prefs_roundtrip():
    asyncio.run(update_prefs(_FakeRequest("u1", {"selectedModel": "cohere:command-a"})))
    out = asyncio.run(get_prefs(_FakeRequest("u1")))
    assert out == {"selectedModel": "cohere:command-a"}


def test_update_merge_parcial():
    asyncio.run(update_prefs(_FakeRequest("u1", {"theme": "dark"})))
    asyncio.run(update_prefs(_FakeRequest("u1", {"language": "pt"})))
    out = asyncio.run(get_prefs(_FakeRequest("u1")))
...
def test_update_ignora_campo_desconhecido():
    result = asyncio.run(
        update_prefs(_FakeRequest("u1", {"theme": "dark", "campo_esquisito": 1}))
    )
...
def test_isola_por_usuario():
    asyncio.run(update_prefs(_FakeRequest("u1", {"theme": "dark"})))
    asyncio.run(update_prefs(_FakeRequest("u2", {"theme": "light"})))
    assert asyncio.run(get_prefs(_FakeRequest("u1"))) == {"theme": "dark"}
    assert asyncio.run(get_prefs(_FakeRequest("u2"))) == {"theme": "light"}


def test_sem_user_cai_no_fallback_local():
    # request.state.user ausente (ex.: middleware não injetou) — usa "local".
    asyncio.run(update_prefs(_FakeRequest(None, {"theme": "dark"})))
    assert asyncio.run(get_prefs(_FakeRequest("local"))) == {"theme": "dark"}
...
def test_body_nao_objeto_retorna_422():
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(update_prefs(_FakeRequest("u1", ["nao", "e", "objeto"])))
