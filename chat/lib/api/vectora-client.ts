...
export type StreamEvent =
  ...
  | { type: "tool_call"; tool_name: string; tool_call_id: string; args_json: string; render_hint?: string; category?: string; destructive?: boolean; icon?: string }
  | { type: "tool_result"; tool_call_id: string; content_json: string; is_error?: boolean }
  | { type: "node"; node: string; status: "started" | "finished"; duration_ms?: number }
  | { type: "ui_metrics"; last_node?: string; last_node_ms?: number; rag_hits?: number; rag_misses?: number; tool_calls?: Record<string, number> }
...
export const getHistory = (thread_id: string): Promise<{ messages: HistoryMessage[] }> =>
