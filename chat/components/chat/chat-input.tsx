 ...
 */
export function ChatInput({ input, onInputChange, onBeforeInput, onSend, onKeyDown, isLoading, isStopping, onStop, userId, attachedFiles, uploadError, inputError, isDragging, onDragOver, onDragLeave, onDrop, onPaste, onRemoveFile, onFileButtonClick, fileInputRef, onFileSelect, textareaRef, isVoiceListening, isVoiceSupported, onVoiceToggle, voiceError, queuedMessages = [] }: ChatInputProps) {
  return (
    <div className="relative">
      ...
      <div className="relative border-t border-border/60 bg-background backdrop-blur-sm">
        <div className="w-full max-w-4xl mx-auto px-3 sm:px-4 py-1.5">
          ...
          {/* Upload Error */}
          {uploadError && <div className="mb-2 text-sm text-destructive bg-destructive/10 px-3 py-2 rounded-md">{uploadError}</div>}

          {/* Voice Error */}
          {voiceError && <div className="mb-2 text-sm text-destructive bg-destructive/10 px-3 py-2 rounded-md">{voiceError}</div>}

          {/* Queued Messages */}
          {queuedMessages.length > 0 && (
            <div className="mb-2 space-y-1.5">
              {queuedMessages.map((msg) => (
                <div key={msg.id} className="flex items-center gap-2 px-3 py-2 bg-muted/50 border border-border/50 rounded-lg text-sm">
                  <div className="flex items-center gap-1.5 text-muted-foreground flex-shrink-0">
                    <svg className="w-3 h-3 animate-pulse" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div className="relative group">
            ...
            <div className="relative">
              ...
              <div className={`relative bg-card/95 backdrop-blur-sm border-2 rounded-xl shadow-2xl transition-all duration-300 group-hover:shadow-3xl group-hover:bg-card/98 group-focus-within:shadow-3xl group-focus-within:bg-white/5 group-focus-within:ring-2 group-focus-within:ring-primary/20 ${isDragging ? "border-primary bg-primary/5 ring-2 ring-primary/30" : "border-border/50 group-hover:border-primary/60 group-focus-within:border-primary/70"}`} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
                {isDragging && (
                ...
                <div className="flex items-end gap-2 px-3 py-1.5">
                  ...
                  {!isLoading && (
                    <Button onClick={onFileButtonClick} variant="ghost" size="sm" disabled={isLoading || !userId} className="group h-9 w-9 p-0 mb-0.5 rounded-full bg-muted/50 hover:bg-primary/10 text-muted-foreground hover:text-primary border-0 flex-shrink-0 transition-all duration-200 hover:scale-105 active:scale-95" type="button" title="Attach files (images, code, logs)">
                  ...
                  <Textarea ref={textareaRef} value={input} onChange={(e) => onInputChange(e.target.value)} onBeforeInput={onBeforeInput} onKeyDown={onKeyDown} onPaste={onPaste} maxLength={MAX_INPUT_CHARS} placeholder={!userId ? "Initializing..." : isLoading ? "Type your next message..." : "Ask me anything about LangChain..."} className="relative z-10 min-h-[36px] max-h-[240px] resize-none bg-transparent border-0 w-full px-3 py-2 text-sm leading-relaxed text-foreground placeholder:text-muted-foreground focus-visible:ring-0 focus-visible:ring-offset-0 transition-all duration-200 break-words custom-scrollbar" disabled={!userId} rows={1} />

                  {isVoiceSupported && onVoiceToggle && <VoiceInputButton isListening={isVoiceListening ?? false} disabled={!userId} onClick={onVoiceToggle} size="sm" />}
          ...
          {inputError && <div className="mt-1 px-2 text-xs text-destructive">{inputError}</div>}
