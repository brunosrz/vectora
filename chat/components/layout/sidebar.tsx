...
export const Sidebar = memo(function Sidebar({ isCollapsed, onToggle, threads, currentThreadId, onSelectThread, onDeleteThread, isLoading = false }: SidebarProps) {
        ...
        <div className="mt-4 px-3 first:mt-0">
          ...
          <div className="space-y-2">
            {groupThreads.map((thread) => {
              ...
              return (
                <div key={thread.thread_id} className={`group flex items-center gap-3 px-3 py-2.5 text-sm w-full rounded-lg transition-all duration-200 cursor-pointer shadow-depth-xs ${thread.thread_id === currentThreadId ? "bg-[#7FC8FF]/15 text-sidebar-foreground shadow-depth-sm border border-[#7FC8FF]/40" : "text-sidebar-foreground"}`} onClick={() => handleSelectThread(thread.thread_id)}>
                  <div className="flex-1 min-w-0">
                    <div className="truncate font-medium">{title}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">{getRelativeTime(threadDate)}</div>
                  </div>
                  <button onClick={(e) => handleDeleteThread(thread.thread_id, e)} className="opacity-0 group-hover:opacity-100 transition-all duration-200 p-1 rounded-md hover:bg-destructive/10">
      ...
      <aside className="hidden md:flex w-56 bg-gradient-to-b from-sidebar via-sidebar-light to-sidebar-lighter border-r border-border/60 flex-col shadow-depth-md">
        ...
        <div className="px-3 py-2 bg-gradient-to-r from-sidebar-accent/5 via-transparent to-transparent">
          <div className="relative group">
            ...
            <Input type="text" placeholder="Search threads..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-8 h-10 text-sm bg-background/80 backdrop-blur-sm border-border/40 focus:border-primary/60 focus:bg-background/90 focus:shadow-sm transition-all duration-200 shadow-sm hover:shadow-md hover:bg-background/90 rounded-lg" />
            {searchQuery && (
              <button type="button" onClick={handleClearSearch} className="absolute right-3 top-1/2 transform -translate-y-1/2 z-10 text-muted-foreground/60 hover:text-foreground transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/30 rounded-full p-0.5 hover:bg-muted/50" aria-label="Clear search">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
          ...
          {isLoading ? (
            ...
            <div className="mt-4 px-3 space-y-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="px-3 py-2.5 rounded-lg" style={{ opacity: 1 - i * 0.12 }}>
        ...
        <div className="bg-gradient-to-t from-sidebar-accent/10 via-sidebar-accent/5 to-transparent pt-2 pb-0 space-y-0">
          <a href="https://github.com/brunosrz/vectora" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 text-sm text-sidebar-foreground transition-all duration-300 ease-out hover:bg-sidebar-accent/10 group">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium leading-tight transition-colors duration-300 group-hover:text-sidebar-primary/90">Documentação</div>
              <div className="text-[10px] text-muted-foreground leading-tight transition-colors duration-300 group-hover:text-muted-foreground/80">GitHub · Vectora</div>
            </div>
          </a>

          <a href="https://github.com/brunosrz/vectora/issues" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 text-sm text-sidebar-foreground transition-all duration-300 ease-out hover:bg-sidebar-accent/10 group">
            <div className="h-6 w-6 rounded-full bg-sidebar-primary/20 flex items-center justify-center shadow-sm shrink-0">
              <MessageSquare className="w-3 h-3 text-sidebar-primary" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium leading-tight transition-colors duration-300 group-hover:text-sidebar-primary/90">Feedback</div>
              <div className="text-[10px] text-muted-foreground leading-tight transition-colors duration-300 group-hover:text-muted-foreground/80">Reportar problema</div>
            </div>
          </a>
