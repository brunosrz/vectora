...
const getAllPaths = (includePrivate: boolean): string[] =>
  Object.keys(CODEBASE_REPOS).flatMap((cat) =>
    getCategoryPaths(cat as CodebaseCategory, includePrivate),
  );
...
export function AgentSettings({
  ...
  const [recursionLimitInput, setRecursionLimitInput] = useState(
    (config.recursionLimit ?? 100).toString(),
  );
  ...
  useEffect(() => {
    ...
    if (!isModelAllowed(config.model as ModelOption)) {
      ...
      console.warn(
        `Model ${config.model} not allowed in current deployment. Resetting to ${defaultModel}`,
      );
    ...
    if (!isAgentAllowed(config.agentType)) {
      ...
      console.warn(
        `Agent ${config.agentType} not allowed in current deployment. Resetting to ${defaultAgent}`,
      );
  ...
  const handleRecursionLimitChange = useCallback(
    (value: string) => {
  ...
  const selectCategory = useCallback(
    (category: CodebaseCategory) => {
      updateRepos([
        ...new Set([
          ...selectedRepos,
          ...getCategoryPaths(category, includePrivate),
        ]),
      ]);
    },
          ...
          <TooltipContent side="bottom" className="max-w-xs">
            <div className="space-y-1 text-xs">
              <div>
                ...
                {getAgentShortDisplayName(config.agentType)}
              </div>
              <div>
                <span className="font-semibold">Model:</span>{" "}
                {getModelDisplayName(config.model as ModelOption)}
              </div>
              <div>
                <span className="font-semibold">Recursion Limit:</span>{" "}
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-1.5">
                <div className="flex items-center justify-between">
                  <Label className="text-xs">
                    Repositories{isPublicOnly && " (public only)"}
                  </Label>
                  <div className="flex gap-2 text-[10px]">
                    <button
                      type="button"
                      onClick={() => updateRepos(getAllPaths(includePrivate))}
                ...
                <div className="space-y-2">
                  {(
                    Object.entries(CODEBASE_REPOS) as [
                      CodebaseCategory,
                      (typeof CODEBASE_REPOS)[CodebaseCategory],
                    ][]
                  ).map(([category, { label }]) => {
                    const repos = getRepos(category, includePrivate);
                    if (repos.length === 0) return null;
                    const paths = getCategoryPaths(category, includePrivate);
                    const allSelected = paths.every((p) =>
                      selectedRepos.includes(p),
                    );
                    const anySelected = paths.some((p) =>
                      selectedRepos.includes(p),
                    );
                    return (
                      <div key={category}>
            ...
            <div className="grid gap-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Maximum number of iterations the agent can perform (default:
                100)
              </p>
            </div>
          </div>
