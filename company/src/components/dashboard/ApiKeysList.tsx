...
function CreateKeyModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-brand-700 bg-brand-900 p-6 shadow-xl">
        {secret ? (
          <>
            ...
            <div className="flex items-center gap-2 rounded-xl border border-brand-500/30 bg-brand-950 px-4 py-3">
              ...
              <button
                ...
                className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:text-white hover:bg-brand-800 transition-colors"
              >
            ...
            <button
              onClick={onClose}
              className="mt-4 w-full rounded-xl border border-brand-700 py-2.5 text-sm font-medium text-slate-300 hover:border-brand-500 hover:text-white transition-all"
            >
          ...
          <>
            ...
            <div className="flex gap-2">
              <button
                onClick={onClose}
                className="flex-1 rounded-xl border border-brand-700 py-2.5 text-sm font-medium text-slate-400 hover:text-white transition-all"
              >
                {m.form_cancel()}
              </button>
              <button
                ...
                className="flex-1 rounded-xl bg-brand-500 py-2.5 text-sm font-semibold text-white hover:bg-brand-400 disabled:opacity-40 transition-all"
              >
                {mutation.isPending
                  ? m.form_submitting()
...
export default function ApiKeysList() {
  ...
  if (isLoading)
    return (
      <div className="h-32 max-w-3xl rounded-xl bg-brand-800/30 animate-pulse" />
    );
    ...
    <div className="max-w-3xl">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl bg-brand-500 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-400 transition-all"
        >
      ...
      {keys?.length === 0 ? (
        <div className="rounded-xl border border-brand-700 bg-brand-800/10 p-8 text-center text-sm text-slate-500">
          {m.apikeys_empty()}
        </div>
      ) : (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {key.scopes.map((s: string) => (
                          <span
                            key={s}
                            className="rounded border border-brand-700 bg-brand-900 px-1.5 py-0.5 text-xs text-slate-400"
                          >
                            {s}
                          </span>
                        ))}
