...
| Allowlist de env pro subprocess MCP local       | Hermes                                | Vectora ainda herda `os.environ` inteiro no subprocess MCP                                       |
...
| Capability token / dedup de subagente nativo    | Hermes                                | Ausente no motor de subagente nativo do Vectora                                                  |
| Liveness ativa (heartbeat + cancelamento real)  | Hermes                                | Vectora só tem watchdog passivo por timeout no motor nativo, sem cancelamento sob pedido         |
| Validação RBAC subagente vs pai                 | Empate                                | Os dois filtram tools do subagente contra o escopo do chamador                                   |
| Goal-mode (Ralph loop)                          | Hermes                                | Ausente no Vectora — nenhuma ocorrência de judge/quality-gate/turn-budget persistente            |
...
| Composição nomeada de toolsets                  | Hermes                                | Vectora usa concatenação estática de listas Python, sem `includes` nomeado nem detecção de ciclo |
...
| **Allowlist de env pro subprocess MCP local** | Maduro                                                                                                                                                       | **Ausente** — `backend/tools/mcp.py::_build_connections()` não passa `env=` explícito, herda `os.environ` inteiro do processo pai (API keys de LLM inclusive) pro subprocess MCP                                               |

**Conclusão**: Vectora lidera com folga grande — a pilha de isolamento OS-level simplesmente não tem paralelo no Hermes. O único gap real e não fechado é a allowlist de env do subprocess MCP.
...
| Capacidade                                            | Hermes                                                                               | Vectora                                                                                                                                                                                                                                                           |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Capability token (anti-forge de handle)               | **Maduro** — HMAC-SHA256 por processo, `hmac.compare_digest`                         | **Ausente no motor nativo** — `backend/engine/subagents.py::run_subagent()` roda inline sem handle assinado. O caminho de produção mais antigo (`backend/tools/background.py::schedule_subagent_task`) já tem capability token, mas isso não cobre o motor nativo |
| Dedup por correlation-id                              | **Maduro**                                                                           | **Ausente no motor nativo** — mesma ressalva acima                                                                                                                                                                                                                |
| Liveness ativa (heartbeat + cancelamento sob demanda) | **Maduro** — `request_hard_interrupt()` cancela de verdade sob pedido                | **Só timeout passivo** — `_watch_liveness()` cancela por inatividade, não por pedido explícito                                                                                                                                                                    |
...
| Goal-mode (Ralph loop)                                | **Muito maduro** — judge LLM, quality gates determinísticos, turn budget, auto-pause | **Ausente, sem vestígio de trabalho** — nenhuma ocorrência de "goal"/"judge"/"quality_gate" em `backend/` fora do comentário que descreve a ausência                                                                                                              |

**Conclusão**: maior gap real e ainda aberto do comparativo inteiro. O motor nativo de subagentes do Vectora está menos protegido do que o caminho de produção mais antigo que ele deve substituir — precisa de hardening (capability token, dedup, liveness ativa real) antes de assumir produção, não depois.
...
| Capacidade                                                    | Hermes                                                                                                              | Vectora                                                                                                                                              |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Composição nomeada de toolsets (`includes` recursivo + ciclo) | **Maduro** — `toolsets.py` (1040 linhas), `resolve_toolset()` com detecção de ciclo                                 | **Ausente** — `souls.py` usa concatenação estática de listas Python (`FS_TOOLS + GIT_TOOLS + ...`)                                                   |
...
**Conclusão**: Hermes lidera com folga grande em discovery de skills (10 fontes vs 1). Vectora tem uma vantagem pontual em discovery de MCP servers especificamente (escopo mais estreito, mas bem desenhado). Composição de toolsets segue como gap real, tecnicamente dependente do motor nativo estar de fato em produção.
...
- Hardening do motor nativo de subagentes: capability token, dedup por
  ...
  por timeout).
- Composição nomeada de toolsets com `includes` recursivo e detecção de
